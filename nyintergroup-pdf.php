...
function attachPdfMeetingData($regions) {
	...
	foreach ($meetings as $meeting) {
		...
		if (!array_key_exists($key, $rows[$meeting['region_id']])) {
			$parts = explode(', ', $meeting['formatted_address']);// formatted address 1773 Beacon St, Brookline, MA 02445, USA 02445
			$rows[$meeting['region_id']][$key] = array(
				'group' => @$meeting['group'],
				'location' => @$meeting['location'],
				'address' => $parts[0] . ', ' . $parts[1] . ', ' . $parts[2],
			//	'postal_code' => substr($parts[2], 3), // we included the zip in address above
...
/* let's just get all the types, not just some types
